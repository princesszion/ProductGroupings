...
# Function definitions remain the same (fetch_images, preprocess_image, extract_features, cluster_images)
# # Function to fetch electronics images from Unsplash
def fetch_images(query, api_key, per_page=20):
    url = f"https://api.unsplash.com/search/photos"
    headers = {"Authorization": f"Client-ID {api_key}"}
...
def display_clusters_enhanced(image_urls_descriptions, labels):
    if len(labels) == 0:
        st.error("No images were clustered.")
        return
    ...
    for label in unique_labels:
        st.subheader(f'Cluster {label + 1}')
        cluster_indices = [idx for idx, cluster_label in enumerate(labels) if cluster_label == label]
        # Display images side by side in an expander for each cluster
        with st.expander(f"View Cluster {label + 1} Images"):
            for idx in cluster_indices:
                url, description = image_urls_descriptions[idx]
                st.image(url, caption=description, width=200)  # Adjust width for better layout

def main():
    st.title("📸 Electronics Image Clustering")
    st.write("Discover visually similar electronics images grouped into clusters.")
    
    api_key = 'MLLiwJrCRyuQwdNDEK5TAyazn8JSqe0IYtFh3T2S4eU'  # Ensure to use your API key
    query = st.text_input("Search Query", "electronics")
    n_clusters = st.sidebar.number_input('Number of Clusters', min_value=2, max_value=20, value=5, step=1)
    fetch_button = st.sidebar.button('Fetch and Cluster Images')
    ...
    if fetch_button:
        ...
        if image_urls:
            ...
            if len(features) > 0:
                ...
                display_clusters_enhanced(image_urls_descriptions, labels)
            else:
                st.error("Failed to extract features. Check if the images were correctly processed.")
        else:
            st.error("Failed to fetch images. Check the API key or query.")
